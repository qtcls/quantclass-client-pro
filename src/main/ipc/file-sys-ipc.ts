...
import { sendErrorToClient } from "@/main/utils/tools.js"
...
import type { SaveRealMarketDataAck } from "@/shared/types/trading.js"
...
async function strategyResultPathHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.STRATEGY_RESULT_PATH, async (_, mode = "backtest") => {
...
async function createRealTradingDirHandler(): Promise<void> {
	ipcMain.handle(
		IPC_CHANNELS.CREATE_REAL_TRADING_DIR,
...
async function selectFileDirHandler(): Promise<void> {
	ipcMain.handle(
		IPC_CHANNELS.OPEN_DIRECTORY_SELECT,
		async (
			_event,
...
async function openDirectoryHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.OPEN_DIRECTORY, async (_event, pathArray) => {
		// path 是一个数组，帮我拼接起来
...
async function openDataDirectoryHandler(): Promise<void> {
	ipcMain.handle(
		IPC_CHANNELS.OPEN_DATA_DIRECTORY,
...
async function openUserDirectoryHandler(): Promise<void> {
	ipcMain.handle(
		IPC_CHANNELS.OPEN_USER_DIRECTORY,
		async (_event, pathArray: string[] | string = []) => {
...
function openUrlHandler() {
	// -- 桥两侧均以 ipcRenderer.send 发送 → 用 on 接收（历史上还有个无调用方的 handle 注册，已并掉）
...
async function saveRealMarketDataHandler(): Promise<void> {
	ipcMain.handle(
		IPC_CHANNELS.SAVE_REAL_MARKET_DATA,
		async (_, data: Record<string, any>): Promise<SaveRealMarketDataAck> => {
			// -- S2b：原子全量替换（rStore.store = data）——替换语义并入 save，删去 renderer
			// -- 侧独立 clean（避免 clean 成功 / save 失败留空文件）。失败回滚到写前快照；
			// -- 永不 reject（异常转 ok:false，fail-closed 不阻塞 renderer enable 链）。
			let prev: Record<string, unknown>
			try {
				prev = structuredClone(rStore.store)
			} catch (error) {
				// -- 连写前快照都取不到 → 不冒险写新数据
				logger.error(`[real_market] 读取回滚快照失败，拒绝写入: ${error}`)
			...
			try {
				...
				try {
					...
					logger.error(`[real_market] 回滚失败: ${rollbackError}`)
				}
				logger.error(`[real_market] 落盘失败: ${error}`)
				return { ok: false, reason: "exception", error: String(error) }
			}
		},
	)
...
async function cleanRealMarketDataHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.CLEAN_REAL_MARKET_DATA, async (_Ipc) => {
...
async function clearRealMarketDataHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.CLEAR_REAL_MARKET_DATA, async () => {
		rStore.clear()
	})
}
...
async function importSelectStockHandler(): Promise<void> {
	...
	ipcMain.handle(IPC_CHANNELS.IMPORT_SELECT_STOCK, async (_, configFilePath: string) => {
		try {
...
async function importFusionHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.IMPORT_FUSION, async (_, configFilePath: string) => {
		try {
			// 获取实盘路径
			const realTradingPath = await store.getAllDataPath(["real_trading"], true)
			let backtestName = ""
		...
		async (_, csvfileName = "最新选股结果", mode = "backtest") => {
			...
			try {
				...
				const backtestName = await store.getValue(
					`${libraryType === "pos" ? POS_MGMT_STRATEGY_CONFIG : SELECT_STOCK_STRATEGY_CONFIG}.backtest_name`,
					"策略库",
				)
...
async function readChangelogHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.READ_CHANGELOG, async () => {
		try {
...
async function importPositionHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.LOAD_POSITION_JSON, async (_, filename: string) => {
		try {
...
async function clearFactorCacheHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.CLEAR_FACTOR_CACHE, async () => {
		try {
...
async function deletePeriodOffsetHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.DELETE_PERIOD_OFFSET, async () => {
		try {
