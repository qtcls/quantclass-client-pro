...
function syncWebUserInfoHandler(): void {
	ipcMain.on(IPC_CHANNELS.SYNC_USER_STATE, (_event, WebUserInfo: WebUserInfo) => {
...
async function getUserAccountHandler(): Promise<void> {
	ipcMain.handle(IPC_CHANNELS.GET_USER_ACCOUNT, async (_event, isForce = false) => {
...
async function clearWebUserInfoHandler(): Promise<void> {
	ipcMain.on(IPC_CHANNELS.CLEAR_USER_STATE, async () => {
